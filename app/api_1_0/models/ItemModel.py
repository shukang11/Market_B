...
class ItemModel(db.Model):
    ...
    item_id = Column(INTEGER, Sequence(increment=1, start=1, name='item_spu_id_seq'), primary_key=True)  # id
    # 商品分类信息(用于定位)
    ...
    item_modified = Column(DATE(), nullable=True, default=datetime.datetime.now())  # 修改时间
    item_vertical_market = Column(SMALLINT, nullable=False, default=0)  # 垂直市场,暂时只有一个
    item_status = Column(SMALLINT(1), nullable=False, default=1)  # 状态(0 商家确认 1 屏蔽 2 后台确认 3 未确认 -1 删除)
    item_level = Column(SMALLINT, nullable=False, default=1)  # 产品级别
    item_rate_num = Column(INTEGER, nullable=False, default='0')  # 产品评分
    item_sale_num = Column(INTEGER, nullable=False, default=0)  # 产品销量
    ite_rest = Column(INTEGER, nullable=False, default=0)# 商品库存
    ...
    item_template_id = Column(SMALLINT, nullable=False, default=1)  # 模板id
    item_commodity_id = Column(INTEGER, nullable=False)  # 品类id、
    item_create_time = Column(DATE, nullable=False, default=datetime.datetime.now())  # 创建时间

    # 产品属性相关
    item_props = Column( String(255), nullable=False)  # 产品的关键属性 对应表-> bao__unique_property
